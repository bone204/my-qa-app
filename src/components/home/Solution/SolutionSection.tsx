          ...
          {solutionNodes.map((node, index) => (
            <React.Fragment key={node.id}>
              <motion.div
                ...
                className="relative w-full max-w-md bg-white/10 backdrop-blur-md p-6 rounded-4xl border border-white/20 shadow-sm"
              >
                <div className="flex items-start gap-4">
                  ...
                  <div className="flex-1 space-y-3">
                    <h3 className="text-base font-bold text-white select-none">{node.title}</h3>
                    <div className="flex flex-wrap gap-2">
                      {node.subItems.map((sub) => (
                        <div 
                          key={sub.label} 
                          className="flex items-center gap-1.5 border rounded-full px-2.5 py-1 transition-colors"
                          style={{ 
                            backgroundColor: `${sub.color}15`,
                            borderColor: `${sub.color}33`
                          }}
                        >
                          <sub.icon className="w-3 h-3" style={{ color: sub.color }} />
                          <span className="text-[10px] font-semibold text-white/70 uppercase tracking-wide">{sub.label}</span>
                        </div>
                      ))}
                    </div>
                  </div>
